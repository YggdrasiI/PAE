...
def innerXML(s):
  ...
  if beginPos == -1 or endPos == -1:
    return s
  if endPos < beginPos:
    return s[:endPos] + s[beginPos+1:]
  else:
    return s[beginPos+1:endPos]

def ReadScenarioDescription(sFile="RiseOfEgypt.xml", bSkipStartingPoints=False):
  szenario = {"Image" : None,
      "Description" : None,
      "Title" : None
      }
  ...
  if( len(sFile) < 5 ):
    return szenario
  try:
    myFile = open("Mods/PieAncientEuropeV/Assets/XML/Scenarios/" + sFile)
    line = myFile.readline()
    while line:
      if "<Image>" in line:
        szenario["Image"] = innerXML(line)
      elif "<Title>" in line:
        szenario["Title"] = innerXML(line)
      elif "<Description>" in line:
        szenario["Description"] = innerXML(line)
        bTagEnds = ("</Description>" in line)
        while not bTagEnds:
          line =myFile.readline()
          if not line:
            break
          ...
          bTagEnds = ("</Description>" in line)
      elif "<StartingPoints>" in line:
        if bSkipStartingPoints:
          break
        ReadStartingPoints(myFile, szenario)
...
# Eingabe wird etwas umsortiert. Jeder Nation wird die Menge ihrer Startpunkte zugeordnet.
def ReadStartingPoints(myFile, szenario):
  ret = szenario.setdefault("StartingPoints", {})
  ...
  while line:
    if "</StartingPoint>" in line:
      sps.append(sp)
      sp = {}
    elif "<StartName>" in line:
      startName = innerXML(line)
      if len(startName) > 0:
        sp["StartName"] = startName
    elif "<LeaderName>" in line:
      sp["LeaderName"] = innerXML(line)
    elif "<StartX>" in line:
      sp["StartX"] = int(innerXML(line))
    elif "<StartY>" in line:
      sp["StartY"] = int(innerXML(line))
    elif "<LeaderType>" in line:
      sp["LeaderType"] = innerXML(line)
    elif "<Civilization>" in line:
      sp["Civilization"] = innerXML(line)
    elif "<Description>" in line:
      sp["Description"] = innerXML(line)
    elif "<ShortDescription>" in line:
      sp["ShortDescription"] = innerXML(line)
    elif "<Adjective>" in line:
      sp["Adjective"] = innerXML(line)
    elif "<DefaultPlayerColor>" in line:
      sp["DefaultPlayerColor"] = innerXML(line)
    elif "<City>" in line:
      sp.setdefault("Cities",[]).append( innerXML(line) )
    elif "</StartingPoints>" in line:
      break
    line = myFile.readline()

  for sp in sps:
    sp["PlayerID"] = -1 # is an unused spot
    ret.setdefault(sp["Civilization"],[]).append(sp)


# Fügt die Standardstartpunkte des Scenarios hinzu
def AttachCurrentStartingPoints( szenario, playersDesc):
  ret = szenario.setdefault("StartingPoints", {})
  sps = []
  iPl = 0
  for pl in playersDesc:
    if pl.leaderType == None or pl.leaderType == "NONE" or pl.leaderType == "LEADER_BARBARIAN":
      continue
    if pl.szLeaderName != "": 
      name = localText.getText(str(pl.szLeaderName), ()) 
    else:
      name = localText.getText(str(pl.leaderType), ()) 

    sp = { "StartName" : ("Default Spot %2d (%s)"%(iPl, name )) ,
        "LeaderName" : pl.szLeaderName,
        "StartX" : pl.iStartingX,
        "StartY" : pl.iStartingY,
        "LeaderType" : pl.leaderType,
        "Civilization" : pl.civType,
        "Description" : pl.szCivDesc,
        "ShortDescription" : pl.szCivShortDesc,
        "Adjective" : pl.szCivAdjective,
        "DefaultPlayerColor" : pl.color,
        "Cities" : pl.aszCityList,
        "PlayerID" : iPl,
        }
    iPl += 1
    sps.append(sp)
  ...
  for sp in sps:
    ret.setdefault(sp["Civilization"],[]).insert(0,sp)
