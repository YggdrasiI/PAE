  ...
  def read(self, f):
    ...
    if (parser.findNextTokenValue(f, "BeginPlayer")!=-1):
      while (true):
        ...
        if v!=-1:
          self.bRandomStartLocation = (v=="true")
          continue
      ...
      if unit:
        ...
        if self.isSleep:
          if unit.canFortify(unit.plot()):
            #unit.getGroup().setActivityType(ActivityTypes.ACTIVITY_HOLD) #wrong, hold just operate in same turn
            unit.getGroup().setActivityType(ActivityTypes.ACTIVITY_SLEEP) # Schlaf wirkt für FORTIFY-Einheiten nicht in der nächsten Runde
            unit.getGroup().pushMission (MissionTypes.MISSION_FORTIFY,0,0,0,False,False,MissionAITypes.NO_MISSIONAI,unit.plot(),unit)
          else:
...
## Platy Builder ##
  def getAssignedStartingPlots(self):
    for iPlayerLoop in xrange(len(self.playersDesc)):
      ...
      if (pPlayer.getLeaderType() != -1 and pWBPlayer.bRandomStartLocation ):
  ...
  ## Platy Builder ##
    for iPlayerLoop in xrange(len(self.playersDesc)):
      ...
      # Extend Citynames by civilization specific ones
      if( pWBPlayer.civType != "NONE" ):
        civInfo = gc.getCivilizationInfo(gc.getInfoTypeForString(pWBPlayer.civType))
        for i in xrange(civInfo.getNumCityNames()):
          cityName = civInfo.getCityNames(i)
          if not cityName in pWBPlayer.aszCityList:
            pPlayer.addCityName(cityName)
  ...
  def read(self, fileName):
    ...
    if bUseExtraScreen:
      # Unload previous szenario description
      CvScreensInterface.startingpointsScreen.initScenarioPage(None)
...
#        self.playersDesc[iNumPlayers2-1].team = barbTeam
  ...
  ## Platy Builder ##
    ...
    if bUseExtraScreen:
      # Load szenario description and setup starting point screen
      if len(self.plotDesc) > 0:
        szenarioFile = self.plotDesc[0].szScriptData + ".xml"
        szenario = CvUtil.ReadScenarioDescription(szenarioFile)
        CvUtil.AttachCurrentStartingPoints( szenario, self.playersDesc)
        if( len(str(szenario.get("description","")).strip()) == 0 ):
          szenario["description"] = self.gameDesc.szDescription
        CvScreensInterface.startingpointsScreen.initScenarioPage(szenario, True)
    return 0
...
def CutTag(s):
  ...
  if beginPos == -1 or endPos == -1:
    return "-1"
  return s[beginPos+1:endPos]

def ReadAlternativeCivNames(civFile="StartingCivNames.xml"):
  myFile = open("Mods/PieAncientEuropeV/Assets/XML/Misc/" + civFile)
  civNames = {} # ["Civ Name Key"] => ["List of Alternatives"]
  for current_line in myFile.readlines():
    if "<Type>" in current_line:
      CurType = CutTag(current_line)
      civNames[CurType] = []
    elif "<Alternative" in current_line:
      altName = CutTag(current_line)
      if altName is not None and AltName != "":
        civNames[CurType].append(altName)
  myFile.close()
  return civNames

def ApplyAlternativeCivNames(civNames, pWBPlayers):
  player = gc.getPlayer(iCivID)
  for iPlayerLoop in xrange(len(pWBPlayers)):
    pPlayer = gc.getPlayer(iPlayerLoop)
    pWBPlayer = playersWB[iPlayerLoop]
    #curDesc = pWBPlayer.szCivDesc
    civType = pWBPlayer.civType
    sameCivType = filter(lambda x: x.civType==civType , pWBPlayers)
    if len(sameCivType)>1:
      sameCivType.pop(0) # skip renaming for first entry
      altNames = civNames.get( pWBPlayer.civType, [])
      while len(altNames)>0 and len(sameCivType)>0:
        altName = altNames.pop(0)
        cloneCiv = sameCivType.pop(0)
        cloneCiv.szCivDesc = altName
